...
import { Camera, Image as ImageIcon, X, Plus, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="w-full bg-white/80 backdrop-blur-xl border-b border-slate-200/60 sticky top-0 z-30 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.04)]">
      <div className="max-w-[1500px] mx-auto p-4 md:p-6 lg:px-10">
        <div className="bg-slate-50/50 rounded-[2.5rem] p-4 border border-white shadow-inner flex items-start gap-6">
          ...
          <div className="flex-1 flex flex-col gap-4 min-w-0">
            ...
            <div className="relative group">
              <textarea
                ...
                className="w-full bg-white border border-slate-100 rounded-[2rem] px-8 py-6 text-xl font-bold text-slate-800 focus:outline-none focus:ring-4 focus:ring-marca-primario/5 transition-all resize-none placeholder:text-slate-300 shadow-sm leading-tight"
                style={{ minHeight: 90 }}
              />
              ...
              <div className="absolute right-6 bottom-4 flex items-center gap-2">
                ...
                <button 
                  ...
                  className={cn(
                    "p-3 text-slate-400 hover:text-marca-primario transition-all rounded-2xl hover:bg-white active:scale-90 border border-transparent hover:border-slate-100",
            ...
            <div className="flex flex-wrap items-center gap-x-10 gap-y-5 px-4 py-1">
              ...
              {imagenes.length > 0 && (
                <div className="flex items-center gap-3 animate-in fade-in slide-in-from-left-2 duration-300">
                   ...
                   <div className="flex gap-2.5">
                    {imagenes.map((img) => (
                      <div key={img.id} className="relative w-16 h-16 rounded-[1.25rem] overflow-hidden border-2 border-white shadow-md ring-1 ring-slate-100 group transition-all hover:scale-105">
                    ...
                    {imagenes.length < 3 && (
                      <button 
                        onClick={() => fileInputRef.current?.click()}
                        className="w-16 h-16 rounded-[1.25rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-300 hover:border-slate-400 hover:text-slate-400 transition-all bg-white/50 active:scale-95"
                      >
                        <Plus size={20} />
                      </button>
                    )}
                ...
                <div className="flex gap-1.5">
                  {Object.entries(AREA_MAP).map(([key, label]) => (
                    <button key={key} onClick={() => setArea(key)}
                      className={cn("px-4 py-2 rounded-xl text-[11px] font-black transition-all uppercase tracking-tighter border",
                ...
                <div className="flex gap-1.5">
                  {Object.entries(LINEA_MAP).map(([key, val]) => (
                    <button key={key} onClick={() => setLinea(key)}
                      className={cn("flex items-center gap-2 px-4 py-2 rounded-xl text-[11px] font-black transition-all uppercase tracking-tighter border",
                      ...
                      <LineIconSelector type={key} size={18} />
                      {val.label}
                    </button>
                  ))}
                </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {Object.entries(CLASIFICACION_MAP).map(([key, val]) => (
                      <button
                        ...
                        title={val.label}
                        className={cn(
                          "flex items-center gap-2 px-3 h-9 rounded-xl transition-all border whitespace-nowrap",
                          clasificacion === key 
          ...
          <button
            ...
            className={cn(
              "w-28 h-28 rounded-full flex items-center justify-center transition-all active:scale-90 shadow-2xl shrink-0 group relative",
              descripcion.trim() 
          ...
          >
            {submitting ? (
              <Icon name="progress_activity" size="48px" className="animate-spin" />
            ) : (
              <Icon 
                name="add" 
                size="64px" 
