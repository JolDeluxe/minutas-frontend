          ...
          {tipo === 'TAREA' && (
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                 ...
                 {isDesktop ? (
                   <SearchableSelect 
                     options={filteredUsers.map(u => ({ label: u.nombre, value: u.id }))}
                     onChange={(val) => val && toggleResponsable(Number(val))}
                   ...
                   <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-none px-1">
                      {filteredUsers.map(u => (
                        <button key={u.id} onClick={() => toggleResponsable(u.id)} className={cn("flex flex-col items-center gap-1.5 shrink-0 transition-all", responsables.includes(u.id) ? "scale-105" : "opacity-40 grayscale")}>
                           <div className={cn("w-14 h-14 rounded-full border-4 flex items-center justify-center overflow-hidden bg-slate-100", responsables.includes(u.id) ? "border-marca-primario shadow-lg" : "border-transparent")}>
                 ...
                 <div className="flex flex-wrap gap-2 p-2 rounded-2xl bg-slate-50/50 border border-dashed border-slate-200 min-h-[44px]">
                    {responsables.length === 0 && <p className="text-[10px] text-slate-400 italic px-2 py-1">Selecciona al menos un responsable</p>}
                    {responsables.map(uid => {
