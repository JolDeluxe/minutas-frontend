...
export const ToastContainer = () => {
  ...
  const mobileTypes = {
    ...
    'color-estado-resuelto': { icon: 'check_circle', bg: 'bg-emerald-600', text: 'text-white' },
  ...
  // Obtenemos la configuración segura o un fallback de 'info'
  const config = mobileTypes[notification?.type] || mobileTypes.info;
    ...
    <>
      ...
      {!isDesktop && notification && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] w-[90%] max-w-sm animate-in slide-in-from-bottom-5 fade-in duration-300">
          ...
          <div className={`flex items-center justify-between px-4 py-3 rounded-md shadow-xl ${config.bg} ${config.text}`}>
            
            {/* Contenido a la izquierda (Icono de estado + Texto) */}
            <div className="flex items-center gap-3 flex-1">
              <Icon name={config.icon} size="24px" fill={true} />
